...
@section('content')
    ...
    <div class="d-flex mb-4">
        ...
        <h2 class="m-3 mt-0 mb-0">Post edit</h2>
    </div>
        ...
        <div class="col-md-8">
            <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('title_uz') is-invalid @enderror" id="title_uz" name="title_uz" value="{{ $post->title_uz }}">
                    @error('title_uz') <div class="invalid-feedback">{{ $message }} </div> @enderror
                </div>
                <div class="mb-3">
                    <label for="title_en" class="form-label">Title (Eng)</label>
                    <input type="text" class="form-control @error('title_en') is-invalid @enderror" id="title_en" name="title_en" value="{{ $post->title_en }}">
                    @error('title_en') <div class="invalid-feedback">{{ $message }} </div> @enderror
                </div>

                <div class="mb-3">
                    <label for="body_uz" class="form-label">Body (Uz)</label>
                    <textarea name="body_uz" class="form-control ckeditor @error('body_uz') is-invalid @enderror" id="body_uz">{{ $post->body_uz }}</textarea>
                    @error('body_uz') <div class="invalid-feedback">{{ $message }} </div> @enderror
                </div>
                <div class="mb-3">
                    <label for="body_en" class="form-label">Body (Eng)</label>
                    <textarea name="body_en" class="form-control ckeditor @error('body_en') is-invalid @enderror" id="body_en">{{ $post->body_en }}</textarea>
...
{{--                check is_special --}}
                <div class="mb-3">
                    <div class="form-check form-switch">
                        ...
                        <input class="form-check-input" name="is_special"  {{ $post->is_special == 1 ? 'checked' : '' }} value="1" type="checkbox" role="switch" id="flexSwitchCheckDefault">
                    </div>
                </div>
